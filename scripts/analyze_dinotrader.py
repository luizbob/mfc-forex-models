"""
Analyze DinoTrader signals
Entry: Open of next bar after signal
Exit: When EXIT signal appears
"""
import pandas as pd
import numpy as np
...
print("=" * 70)
print("DINOTRADER SIGNAL ANALYSIS")
print("=" * 70)
print(f"Started: {datetime.now()}")

# Load and analyze H1 signals first
TIMEFRAME = 'H1'
...
# Check signal types
print(f"\nSignal types:")
print(signals_df['tipo_sinal'].value_counts())

# Date range
print(f"\nDate range: {signals_df['datetime'].min()} to {signals_df['datetime'].max()}")
...
# Backtest function
def backtest_dinotrader(pair, start_date='2021-01-01', end_date='2025-12-31'):
    """Backtest DinoTrader signals for a pair"""
...
print("\n" + "=" * 70)
print("BACKTESTING (2021-2025)")
print("=" * 70)

all_trades = []
for pair in ALL_PAIRS:
    trades = backtest_dinotrader(pair, '2021-01-01', '2025-12-31')
...
if len(trades_df) > 0:
    ...
    # Hold time analysis
    print("\n--- Hold Time ---")
    print(f"Avg bars held: {trades_df['bars_held'].mean():.1f} ({trades_df['bars_held'].mean():.1f} hours)")
    print(f"Median bars held: {trades_df['bars_held'].median():.0f} hours")
    print(f"Max bars held: {trades_df['bars_held'].max():.0f} hours")
    ...
    # Save results
    trades_df.to_csv(DATA_DIR.parent / 'lstm_model' / 'data' / 'dinotrader_h1_trades.csv', index=False)
    print(f"\nSaved trades to: dinotrader_h1_trades.csv")
