"""
Quality Entry Trader for MT5 (M15 Base)
=======================================
Uses XGBoost to filter quality entries on M15 timeframe.
...
M15 trades are faster than M30:
- More frequent signals
- Best model performance (AUC 0.82, PF up to 8.8)
"""
...
# Strategy Parameters - M15 Quality Entry
MFC_EXTREME_THRESHOLD = 0.5   # Entry zone
QUALITY_THRESHOLD = 0.80      # Minimum quality probability (0.8 = 87.3% quality, 86.1% win rate)
STOCH_PERIOD = 50             # Stochastic period (on M15)
STOCH_EXIT_HIGH = 80
STOCH_EXIT_LOW = 20
MAX_BARS_HOLD = 100           # M15 bars (~25 hours - similar to M30/H1 coverage)
FRIDAY_CUTOFF_HOUR = 12       # No entries after Friday 12:00

# Trading Parameters
LOT_SIZE = 0.01
MAGIC_NUMBER = 150012         # Different from H1 (150010) and M30 (150011)
MAX_SPREAD_PIPS = 5
MAX_POSITIONS = 10
...
# ============================================================================
# QUALITY MODEL MANAGER (M15)
# ============================================================================

class QualityModelManager:
    """Manages the M15 XGBoost quality entry classifier."""
    ...
    def load(self) -> bool:
        """Load the M15 quality classifier model (properly trained)."""
        model_path = self.model_dir / 'quality_xgb_m15_proper.joblib'
        features_path = self.model_dir / 'quality_xgb_features_m15_proper.pkl'

        if not model_path.exists():
            log.error(f"M15 model not found: {model_path}")
            return False

        if not features_path.exists():
            log.error(f"M15 features not found: {features_path}")
            return False
        ...
        try:
            ...
            log.info(f"Loaded M15 quality model (proper): {len(self.feature_cols)} features")
            return True
...
class MFCDataReader:
    ...
    def get_mfc_features(self, base: str, quote: str, direction: str) -> dict:
        """
        Get all MFC features for M15 quality prediction.
        Uses M15 as base timeframe.
        """
        data = self.read()
        if not data:
            return None
        ...
        # Additional M15 features (base timeframe for this model)
        base_m15 = data[base].get('M15', [])

        if len(base_m15) >= 4:
            features['base_vel2_m15'] = base_m15[-2] - base_m15[-4]
            vel1 = base_m15[-2] - base_m15[-3]
            vel2 = base_m15[-3] - base_m15[-4]
            features['base_acc_m15'] = vel1 - vel2
        else:
            features['base_vel2_m15'] = 0
            features['base_acc_m15'] = 0

        # Divergence (M15 based)
        features['divergence'] = features['base_m15'] - features['quote_m15']
        features['vel_divergence'] = features['base_vel_m15'] - features['quote_vel_m15']
...
class MT5Trader:
    ...
    def calculate_stochastic(self, symbol: str, period: int = 50, shift: int = 1) -> float:
        """Calculate Stochastic on M15."""
...
class QualityEntryStrategyM15:
    """M15-based quality entry strategy."""
    ...
    def initialize(self) -> bool:
        log.info("=" * 60)
        log.info("QUALITY ENTRY TRADER (M15 Base)")
        log.info("=" * 60)
        ...
        log.info(f"Max hold: {MAX_BARS_HOLD} M15 bars ({MAX_BARS_HOLD * 0.25:.0f} hours)")
    ...
    def check_entry(self, pair: str, base: str, quote: str) -> str:
        ...
        base_mfc = buy_features['base_m15']  # Use M15 for entry decision
        ...
        if base_mfc <= -MFC_EXTREME_THRESHOLD:
            ...
            if quality_prob >= QUALITY_THRESHOLD:
                log.info(f"{pair} BUY SIGNAL: base_mfc={base_mfc:.3f}, "
                        f"vel={buy_features['base_vel_m15']:.4f}, "
        ...
        if base_mfc >= MFC_EXTREME_THRESHOLD:
            ...
            if quality_prob >= QUALITY_THRESHOLD:
                log.info(f"{pair} SELL SIGNAL: base_mfc={base_mfc:.3f}, "
                        f"vel={sell_features['base_vel_m15']:.4f}, "
                        f"quality={quality_prob:.1%}")
                return 'SELL'
            else:
    ...
    def check_exit(self, pair: str, base: str) -> bool:
        ...
        # Check MFC return to center (using M15)
        data = self.mfc_reader.read()
        if data and base in data:
            base_m15 = data[base].get('M15', [])
            if len(base_m15) >= 2:
                current_mfc = base_m15[-2]
        ...
        # Check timeout (in hours, convert from M15 bars)
        hours_held = self.trader.get_hours_since_entry(symbol)
        max_hours = MAX_BARS_HOLD * 0.25  # M15 bars to hours
    ...
    def process_bar(self):
        ...
        for pair, base, quote in ALL_PAIRS:
            ...
            if self.trader.has_position(symbol):
                ...
                if signal == 'BUY':
                    signals_found += 1
                    self.trader.open_buy(symbol, LOT_SIZE, "M15_QUALITY_BUY")
                elif signal == 'SELL':
                    signals_found += 1
                    self.trader.open_sell(symbol, LOT_SIZE, "M15_QUALITY_SELL")

        if signals_found == 0:
            log.info(f"No signals this bar")

    def wait_for_new_bar(self) -> bool:
        """Wait for new M15 bar."""
        ref_symbol = self.trader.get_symbol(ALL_PAIRS[0][0])

        rates = mt5.copy_rates_from_pos(ref_symbol, mt5.TIMEFRAME_M15, 0, 1)
        if rates is None:
            return False
    ...
    def run(self):
        self.running = True
        log.info("Strategy started. Waiting for M15 bar signals...")

        while self.running:
            try:
                if self.wait_for_new_bar():
                    log.info(f"New M15 bar detected ({self.last_bar_time.strftime('%H:%M')}), waiting for fresh MFC data...")
...
def main():
    strategy = QualityEntryStrategyM15()
