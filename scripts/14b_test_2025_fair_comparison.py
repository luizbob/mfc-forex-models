...
log("=" * 70)
log("2025 TEST - H4 + NO BAD PAIRS + RSI 21")
log("=" * 70)
...
# Data limits per currency (using 2025 end date for all)
DATA_END_DATES = {
    'EUR': '2025-12-21',
    'NZD': '2025-12-21',
    'USD': '2025-12-21',
...
# Pairs to exclude (disabled for RSI testing)
EXCLUDE_PAIRS = []
...
MAX_HOLD_BARS = 250  # Optimal timeout (20.8 hours)
...
for pair, base, quote in ALL_PAIRS:
    ...
    try:
        ...
        price_df['base_vel_h1'] = base_h1.diff(periods=12)  # 12 * 5min = 1 hour velocity
        price_df['quote_vel_h1'] = quote_h1_vel.diff(periods=12)

        # H4 velocity (shifted by 1) - 6 H4 bars = 24 hours
        base_h4 = mfc_h4[base].shift(1).reindex(price_df.index, method='ffill')
        quote_h4 = mfc_h4[quote].shift(1).reindex(price_df.index, method='ffill')
        price_df['base_vel_h4'] = base_h4.diff(periods=72)  # 72 * 5min = 6 H4 bars = 24h
        ...
        # Friday filter - with 250 bar timeout (20.8h), need to stop early
        # Testing showed 06:00 cutoff is BOTH safer AND more profitable than 08:00
        # Friday morning trades (06:00-08:00) have negative expected value
        is_friday_afternoon = (price_df.index.dayofweek == 4) & (price_df.index.hour >= 6)

        # Combined velocity filter: momentum should favor our direction
        # Note: Extended filter was tested but REMOVED - it hurts performance
        # BUY: base rising + quote falling = (base_vel - quote_vel) > 0
        # SELL: quote rising + base falling = (quote_vel - base_vel) > 0
        buy_vel_ok = (price_df['base_vel_h1'] - price_df['quote_vel_h1']) > 0
        sell_vel_ok = (price_df['quote_vel_h1'] - price_df['base_vel_h1']) > 0

        # LSTM divergence + MFC extreme + RSI extreme + H1 velocity filter + Friday filter + Combined Vel filter
        buy_signal = (
            (price_df['base_dir'] == 2) &
        ...
        while i < len(buy_indices):
            ...
            # Entry at Open of NEXT candle (realistic)
            ...
            if exit_mask.any():
                ...
                # TIMEOUT: close at last bar of the 200 bar window
...
if len(all_trades) == 0:
    ...
    for reason in trades_df['exit_reason'].unique():
        ...
        log(f"  {reason}: {len(reason_df)} trades, {reason_df['win'].mean()*100:.1f}% WR, {reason_df['net_pips'].mean():.2f} net avg")
    ...
    # Save trades
    trades_df.to_csv(LSTM_DATA_DIR / 'trades_2025_rsi21_oos.csv', index=False)
    log(f"\n  Trades saved to trades_2025_rsi21_oos.csv")

# ============================================================================
# COMPARISON
# ============================================================================
log("\n" + "=" * 70)
log("6. 2025 RESULTS (H4 + NO BAD PAIRS + RSI 21)")
log("=" * 70)

if len(all_trades) > 0:
    log(f"| Period        | Trades | Win Rate | Net Avg | Total   |")
    log(f"|---------------|--------|----------|---------|---------|")
    log(f"| 2021 OOS      | {len(trades_df):<6} | {trades_df['win'].mean()*100:.1f}%    | {trades_df['net_pips'].mean():+.2f}   | {trades_df['net_pips'].sum():+.0f} |")
