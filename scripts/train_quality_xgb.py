...
log("=" * 70)
log("QUALITY ENTRY XGBoost TRAINING")
log("=" * 70)
...
# Save model
model_path = MODEL_DIR / 'quality_xgb_classifier.joblib'
joblib.dump(model, model_path)
log(f"Saved model to: {model_path}")

# Save feature columns for inference
with open(MODEL_DIR / 'quality_xgb_features.pkl', 'wb') as f:
    pickle.dump(feature_cols, f)
log(f"Saved features to: {MODEL_DIR / 'quality_xgb_features.pkl'}")
...
results = {
    ...
    'config': config,
    'thresholds': {0.5: None, 0.6: None, 0.7: None, 0.8: None, 0.9: None}
}
...
for thresh in [0.5, 0.6, 0.7, 0.8, 0.9]:
    ...
    if high_conf_mask.sum() > 0:
        results['thresholds'][thresh] = {
            'precision': float(y_test[high_conf_mask].mean()),
            'count': int(high_conf_mask.sum())
        }

with open(MODEL_DIR / 'quality_xgb_results.pkl', 'wb') as f:
