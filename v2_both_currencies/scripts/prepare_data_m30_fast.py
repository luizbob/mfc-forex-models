"""
Prepare Quality Entry Dataset - V2 Both Currencies (M30) - FAST VERSION
=======================================================================
...
log("=" * 70)
log("PREPARING QUALITY ENTRY DATASET - V2 BOTH CURRENCIES (M30) - FAST")
log("=" * 70)
...
def process_entries_batch(df, indices, mfc_arr, price_arr, shifted_arr,
    ...
    results['base_vel2_m30'] = df['base_vel2_m30'].values[indices]
    results['base_acc_m30'] = df['base_acc_m30'].values[indices]
    results['quote_vel2_m30'] = df['quote_vel2_m30'].values[indices]
...
def create_dataset_for_pair(base_ccy, quote_ccy):
    ...
    # Store raw M30 for exit tracking (unshifted)
    df['base_m30_raw'] = base_mfc['M30']
    df['quote_m30_raw'] = quote_mfc['M30']
    ...
    for tf in ['M5', 'M15', 'M30', 'H1', 'H4']:
        ...
        if base_raw is not None:
            ...
            if tf == 'M30':
                ...
                # Higher/lower TFs - ffill to M30 grid
    ...
    # Additional M30 momentum features (on already-shifted data)
    df['base_vel2_m30'] = df['base_m30'].diff(2)
    df['base_acc_m30'] = df['base_vel_m30'].diff()
    df['quote_vel2_m30'] = df['quote_m30'].diff(2)
    df['quote_acc_m30'] = df['quote_vel_m30'].diff()

    # Divergence (on shifted M30)
    ...
    base_mfc_arr = df['base_m30_raw'].values  # Raw for future tracking
    quote_mfc_arr = df['quote_m30_raw'].values
    price_arr = df['price'].values
    base_shifted_arr = df['base_m30'].values  # Shifted for entry detection
    quote_shifted_arr = df['quote_m30'].values
