"""
Prepare Quality Entry Dataset - V2 Both Currencies (M30)
========================================================
...
log("=" * 70)
log("PREPARING QUALITY ENTRY DATASET - V2 BOTH CURRENCIES (M30)")
log("=" * 70)
...
RETURN_TARGET = 0.0
MAX_BARS_TO_RETURN = 400  # M30 bars
...
def create_dataset_for_pair(base_ccy, quote_ccy):
    ...
    # This prevents data leakage for both higher TFs (H1, H4) and lower TFs (M5, M15)
    ...
    # Apply shift(1) on M30 grid - use previous M30 bar's data for trigger/features
    for tf in ['m5', 'm15', 'm30', 'h1', 'h4']:
    ...
    # Additional M30 features (momentum for trading timeframe)
    df['base_vel2_m30'] = df['base_m30_shifted'].diff(2)
    df['base_acc_m30'] = df['base_vel_m30'].diff()
    df['quote_vel2_m30'] = df['quote_m30_shifted'].diff(2)
    ...
    buy_mask_base = df['base_m30_shifted'] <= -MFC_EXTREME_THRESHOLD
    ...
    sell_mask_base = df['base_m30_shifted'] >= MFC_EXTREME_THRESHOLD
    ...
    sell_mask_quote = df['quote_m30_shifted'] <= -MFC_EXTREME_THRESHOLD
    ...
    buy_mask_quote = df['quote_m30_shifted'] >= MFC_EXTREME_THRESHOLD
...
def label_entry(df, idx, direction, trigger, trigger_ccy):
    ...
    try:
        ...
        if trigger == 'base':
            ...
            entry_mfc = df.iloc[pos]['quote_m30_shifted']
            future_mfc = df.iloc[pos+1:pos+MAX_BARS_TO_RETURN+1]['quote_m30']
        ...
        features['quote_vel2_m30'] = row['quote_vel2_m30']
        features['quote_acc_m30'] = row['quote_acc_m30']
...
def main():
    ...
    # Save
    output_path = OUTPUT_DIR / 'quality_entry_data_m30_v2.pkl'
    output = {
        'data': final_df,
        'config': {
