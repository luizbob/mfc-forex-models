"""
Prepare Quality Entry Dataset - V2 Both Currencies (M5)
=======================================================
...
log("=" * 70)
log("PREPARING QUALITY ENTRY DATASET - V2 BOTH CURRENCIES (M5)")
log("=" * 70)
...
def create_dataset_for_pair(base_ccy, quote_ccy):
    ...
    # Create base dataframe on M5
    df = pd.DataFrame(index=base_mfc['M5'].index)
    df['base_m5'] = base_mfc['M5']
    df['quote_m5'] = quote_mfc['M5']
    ...
    # This prevents data leakage - M15 bar at 10:15 closes at 10:30,
    # so at M5 10:20 we should only see M15 10:00 (which closed at 10:15)
    for tf in ['M15', 'M30', 'H1', 'H4']:
        if base_mfc[tf] is not None:
            # shift(1) in the timeframe's own resolution, then ffill to M5 grid
    ...
    # Apply shift(1) on M5 grid - use previous M5 bar's data for trigger/features
    # This aligns with: "at bar T, check if T-1 had trigger, enter at T+1"
    for tf in ['m5', 'm15', 'm30', 'h1', 'h4']:
    ...
    # Additional M5 features (momentum for trading timeframe)
    df['base_vel2_m5'] = df['base_m5_shifted'].diff(2)
    df['base_acc_m5'] = df['base_vel_m5'].diff()
    df['quote_vel2_m5'] = df['quote_m5_shifted'].diff(2)
    df['quote_acc_m5'] = df['quote_vel_m5'].diff()

    # Divergence
    df['divergence'] = df['base_m5_shifted'] - df['quote_m5_shifted']
    ...
    buy_mask_base = df['base_m5_shifted'] <= -MFC_EXTREME_THRESHOLD
    ...
    sell_mask_base = df['base_m5_shifted'] >= MFC_EXTREME_THRESHOLD
    ...
    sell_mask_quote = df['quote_m5_shifted'] <= -MFC_EXTREME_THRESHOLD
    ...
    buy_mask_quote = df['quote_m5_shifted'] >= MFC_EXTREME_THRESHOLD
...
def label_entry(df, idx, direction, trigger, trigger_ccy):
    ...
    try:
        ...
        if trigger == 'base':
            entry_mfc = df.iloc[pos]['base_m5_shifted']
            future_mfc = df.iloc[pos+1:pos+MAX_BARS_TO_RETURN+1]['base_m5']
        else:  # quote trigger
            entry_mfc = df.iloc[pos]['quote_m5_shifted']
            future_mfc = df.iloc[pos+1:pos+MAX_BARS_TO_RETURN+1]['quote_m5']
        ...
        exit_pnl_pips = 0.0
        bars_to_exit = MAX_BARS_TO_RETURN
        ...
        features['base_vel2_m5'] = row['base_vel2_m5']
        features['base_acc_m5'] = row['base_acc_m5']
        features['quote_vel2_m5'] = row['quote_vel2_m5']
        features['quote_acc_m5'] = row['quote_acc_m5']
