"""
Quality Entry Trader V2 - Rolling Walk-Forward Model (M5)
=========================================================
Uses the rolling window walk-forward trained model.
Train window: 4 years, rolling forward.
More adaptive to recent market conditions.
...
QUALITY_THRESHOLD = 0.75  # Rolling model performs well at 0.75
MAX_BARS_HOLD = 2400  # M5 bars (~200 hours max)
FRIDAY_CUTOFF_HOUR = 18

# Trading Parameters
LOT_SIZE = 0.01
MAGIC_NUMBER = 150053  # V2 Rolling model magic number
MAX_SPREAD_PIPS = 5
MAX_POSITIONS = 100
...
class TradeLogger:
    ...
    def __init__(self, log_dir: Path):
        ...
        self.trades_file = self.log_dir / "trades_v2roll.csv"
        self._init_file()
...
# ============================================================================
# QUALITY MODEL MANAGER (Rolling Walk-Forward)
# ============================================================================
...
class QualityModelManager:
    ...
    def load(self) -> bool:
        ...
        if not model_path.exists() or not features_path.exists():
            log.error(f"Rolling walk-forward model not found")
            return False
        ...
        try:
            ...
            log.info(f"Loaded Rolling Walk-Forward model: {len(self.feature_cols)} features")
            return True
...
class MFCDataReader:
    ...
    def get_mfc_features(self, base: str, quote: str, direction: str, trigger: str) -> dict:
        """Get features for rolling walk-forward model (includes time features)."""
        data = self.read()
        ...
        for p in self.get_positions():
            if p.symbol == symbol:
                try:
                    ...
                    if len(parts) >= 3 and parts[0] == 'V2ROLL':
...
class QualityEntryStrategyV2Rolling:
    def __init__(self):
    ...
    def initialize(self) -> bool:
        log.info("=" * 60)
        log.info("QUALITY ENTRY TRADER V2 - ROLLING WALK-FORWARD (M5)")
        log.info("=" * 60)
        ...
        log.info(f"Model: Rolling Walk-Forward (4yr train window)")
    ...
    def process_bar(self):
        ...
        for pair, base, quote in ALL_PAIRS:
            ...
            if self.trader.has_position(symbol):
                ...
                if direction:
                    signals += 1
                    comment = f"V2ROLL_{trigger.upper()}_{trigger_ccy}_{direction.upper()}"
...
def main():
    strategy = QualityEntryStrategyV2Rolling()
    try:
