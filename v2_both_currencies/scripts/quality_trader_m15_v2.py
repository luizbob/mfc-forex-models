"""
Quality Entry Trader V2 - Both Currency Triggers (M15)
======================================================
...
# Trading Parameters
LOT_SIZE = 0.01
MAGIC_NUMBER = 150020  # V2 magic number
MAX_SPREAD_PIPS = 5
MAX_POSITIONS = 10
...
class QualityModelManager:
    ...
    def load(self) -> bool:
        model_path = self.model_dir / 'quality_xgb_m15_v2_pnl.joblib'
        features_path = self.model_dir / 'quality_xgb_features_m15_v2_pnl.pkl'

        if not model_path.exists() or not features_path.exists():
            log.error(f"V2 model not found")
            return False
        ...
        try:
            ...
            log.info(f"Loaded V2 quality model: {len(self.feature_cols)} features")
            return True
...
class MFCDataReader:
    ...
    def get_mfc_features(self, base: str, quote: str, direction: str, trigger: str) -> dict:
        """Get features for V2 model (includes trigger_code)."""
        data = self.read()
        ...
        # Momentum features (M15)
        base_m15 = data[base].get('M15', [])
        quote_m15 = data[quote].get('M15', [])

        if len(base_m15) >= 4:
            features['base_vel2_m15'] = base_m15[-2] - base_m15[-4]
            features['base_acc_m15'] = (base_m15[-2] - base_m15[-3]) - (base_m15[-3] - base_m15[-4])
        else:
            features['base_vel2_m15'] = 0
            features['base_acc_m15'] = 0

        if len(quote_m15) >= 4:
            features['quote_vel2_m15'] = quote_m15[-2] - quote_m15[-4]
            features['quote_acc_m15'] = (quote_m15[-2] - quote_m15[-3]) - (quote_m15[-3] - quote_m15[-4])
        else:
            features['quote_vel2_m15'] = 0
            features['quote_acc_m15'] = 0
...
class QualityEntryStrategyV2:
    ...
    def initialize(self) -> bool:
        log.info("=" * 60)
        log.info("QUALITY ENTRY TRADER V2 - BOTH CURRENCIES (M15)")
        log.info("=" * 60)
    ...
    def check_entry(self, pair: str, base: str, quote: str) -> tuple:
        ...
        if len(base_m15) < 2 or len(quote_m15) < 2:
            return None, None, None

        base_mfc = base_m15[-2]  # Shifted
        quote_mfc = quote_m15[-2]
    ...
    def check_exit(self, pair: str, base: str, quote: str) -> bool:
        ...
        if data:
            ...
            if trigger_type == 'base':
                mfc_values = data[base].get('M15', [])
                check_ccy = base
            else:  # quote trigger
                mfc_values = data[quote].get('M15', [])
                check_ccy = quote
            ...
            if len(mfc_values) >= 2:
                ...
                # BASE trigger: BUY when low (-0.5), exit when >= 0
                #               SELL when high (+0.5), exit when <= 0
                # QUOTE trigger: SELL when low (-0.5), exit when quote >= 0
                #                BUY when high (+0.5), exit when quote <= 0
                if trigger_type == 'base':
        ...
        while self.running:
            try:
                if self.wait_for_new_bar():
                    log.info(f"New M15 bar ({self.last_bar_time.strftime('%H:%M')}), waiting for fresh MFC...")
