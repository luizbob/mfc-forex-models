"""
Quality Entry Trader V2 - Both Currency Triggers (M30)
======================================================
...
# Trading Parameters
LOT_SIZE = 0.01
MAGIC_NUMBER = 150021  # V2 M30 magic number
MAX_SPREAD_PIPS = 5
MAX_POSITIONS = 10
...
class QualityModelManager:
    ...
    def load(self) -> bool:
        model_path = self.model_dir / 'quality_xgb_m30_v2_pnl.joblib'
        features_path = self.model_dir / 'quality_xgb_features_m30_v2_pnl.pkl'

        if not model_path.exists() or not features_path.exists():
            log.error(f"V2 M30 model not found")
            return False
        ...
        try:
            ...
            log.info(f"Loaded V2 M30 quality model: {len(self.feature_cols)} features")
            return True
...
class MFCDataReader:
    ...
    def get_mfc_features(self, base: str, quote: str, direction: str, trigger: str) -> dict:
        """Get features for V2 M30 model (includes trigger_code)."""
        data = self.read()
        ...
        # Momentum features (M30 - trading timeframe)
        base_m30 = data[base].get('M30', [])
        quote_m30 = data[quote].get('M30', [])

        if len(base_m30) >= 4:
            features['base_vel2_m30'] = base_m30[-2] - base_m30[-4]
            features['base_acc_m30'] = (base_m30[-2] - base_m30[-3]) - (base_m30[-3] - base_m30[-4])
        else:
            features['base_vel2_m30'] = 0
            features['base_acc_m30'] = 0

        if len(quote_m30) >= 4:
            features['quote_vel2_m30'] = quote_m30[-2] - quote_m30[-4]
            features['quote_acc_m30'] = (quote_m30[-2] - quote_m30[-3]) - (quote_m30[-3] - quote_m30[-4])
        else:
            features['quote_vel2_m30'] = 0
            features['quote_acc_m30'] = 0
...
class QualityEntryStrategyV2:
    ...
    def initialize(self) -> bool:
        log.info("=" * 60)
        log.info("QUALITY ENTRY TRADER V2 - BOTH CURRENCIES (M30)")
        log.info("=" * 60)
    ...
    def check_entry(self, pair: str, base: str, quote: str) -> tuple:
        ...
        if len(base_m30) < 2 or len(quote_m30) < 2:
            return None, None, None

        base_mfc = base_m30[-2]  # Shifted
        quote_mfc = quote_m30[-2]
    ...
    def check_exit(self, pair: str, base: str, quote: str) -> bool:
        ...
        if data:
            ...
            if trigger_type == 'base':
                mfc_values = data[base].get('M30', [])
                check_ccy = base
            else:  # quote trigger
                mfc_values = data[quote].get('M30', [])
                check_ccy = quote
        ...
        max_hours = MAX_BARS_HOLD * 0.5  # 0.5 hours per M30 bar
        ...
        while self.running:
            try:
                if self.wait_for_new_bar():
                    log.info(f"New M30 bar ({self.last_bar_time.strftime('%H:%M')}), waiting for fresh MFC...")
