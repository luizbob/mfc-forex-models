"""
Quality Entry Trader V2 OPTIMIZED - Both Currency Triggers (M5)
===============================================================
Uses the optimized XGBoost model with time features.
Higher trade volume at 0.75 threshold.
...
QUALITY_THRESHOLD = 0.75  # Lower threshold for more trades
MAX_BARS_HOLD = 2400  # M5 bars (~200 hours max)
FRIDAY_CUTOFF_HOUR = 18

# Trading Parameters
LOT_SIZE = 0.01
MAGIC_NUMBER = 150052  # V2 OPTIMIZED magic number
MAX_SPREAD_PIPS = 5
MAX_POSITIONS = 100  # Increased for higher trade volume
...
# ============================================================================
# QUALITY MODEL MANAGER (V2 OPTIMIZED)
# ============================================================================
...
class QualityModelManager:
    ...
    def load(self) -> bool:
        ...
        model_path = self.model_dir / 'quality_xgb_m5_v2_pnl_optimized.joblib'
        features_path = self.model_dir / 'quality_xgb_features_m5_v2_pnl_optimized.pkl'

        if not model_path.exists() or not features_path.exists():
            log.error(f"Optimized model not found at {model_path}")
            return False
        ...
        try:
            ...
            log.info(f"Loaded OPTIMIZED model: {len(self.feature_cols)} features")
            log.info(f"Features: {self.feature_cols}")
            return True
...
class MFCDataReader:
    ...
    def get_mfc_features(self, base: str, quote: str, direction: str, trigger: str, server_time: datetime) -> dict:
        """Get features for OPTIMIZED model (includes time features)."""
        data = self.read()
        ...
        # TIME FEATURES (new for optimized model)
        hour = server_time.hour
        dayofweek = server_time.weekday()
        features['hour_sin'] = np.sin(2 * np.pi * hour / 24)
        features['hour_cos'] = np.cos(2 * np.pi * hour / 24)
        features['dow_sin'] = np.sin(2 * np.pi * dayofweek / 5)
        features['dow_cos'] = np.cos(2 * np.pi * dayofweek / 5)
...
class MT5Trader:
    ...
    def get_server_time(self, symbol: str) -> datetime:
        """Get server time from tick data."""
        ...
        for p in self.get_positions():
            if p.symbol == symbol:
                try:
                    ...
                    if len(parts) >= 3 and parts[0] == 'V2OPT':
...
class QualityEntryStrategyV2Optimized:
    def __init__(self):
    ...
    def initialize(self) -> bool:
        log.info("=" * 60)
        log.info("QUALITY ENTRY TRADER V2 OPTIMIZED (M5)")
        log.info("=" * 60)
    ...
    def check_entry(self, pair: str, base: str, quote: str, server_time: datetime) -> tuple:
        ...
        if base_mfc <= -MFC_EXTREME_THRESHOLD:
            features = self.mfc_reader.get_mfc_features(base, quote, 'buy', 'base', server_time)
            if features:
        ...
        if base_mfc >= MFC_EXTREME_THRESHOLD:
            features = self.mfc_reader.get_mfc_features(base, quote, 'sell', 'base', server_time)
            if features:
                quality = self.model_manager.predict_quality(features)
                if quality >= QUALITY_THRESHOLD:
        ...
        if quote_mfc <= -MFC_EXTREME_THRESHOLD:
            features = self.mfc_reader.get_mfc_features(base, quote, 'sell', 'quote', server_time)
            if features:
                quality = self.model_manager.predict_quality(features)
                if quality >= QUALITY_THRESHOLD:
        ...
        if quote_mfc >= MFC_EXTREME_THRESHOLD:
            features = self.mfc_reader.get_mfc_features(base, quote, 'buy', 'quote', server_time)
            if features:
                quality = self.model_manager.predict_quality(features)
                if quality >= QUALITY_THRESHOLD:
    ...
    def process_bar(self, server_time: datetime):
        ...
        for pair, base, quote in ALL_PAIRS:
            ...
            if self.trader.has_position(symbol):
                ...
                direction, trigger, trigger_ccy = self.check_entry(pair, base, quote, server_time)
                if direction:
                    signals += 1
                    comment = f"V2OPT_{trigger.upper()}_{trigger_ccy}_{direction.upper()}"
            ...
            try:
                if self.wait_for_new_bar():
                    ...
                    if self.mfc_reader.wait_for_fresh_data(self.last_bar_time, timeout=120):
                        # Get server time for time features
                        ref_symbol = self.trader.get_symbol(ALL_PAIRS[0][0])
                        server_time = self.trader.get_server_time(ref_symbol)
                        self.process_bar(server_time)
                    else:
...
def main():
    strategy = QualityEntryStrategyV2Optimized()
    try:
