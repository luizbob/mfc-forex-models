"""
Hyperparameter Tuning V2 - Different Parameter Space
=====================================================
Explores parameters NOT covered in the original tuning:
...
Saves to: quality_xgb_m5_v2_tuned_v2.joblib
"""
import warnings
...
# ============================================================================
# CONFIGURATION - V2 (DIFFERENT FROM ORIGINAL)
# ============================================================================

# Time-series validation settings (same as original for fair comparison)
TRAIN_YEARS = 4
VAL_MONTHS = 6
N_VALIDATION_FOLDS = 3
EMBARGO_HOURS = 24

# V2 HYPERPARAMETER GRID - Different parameters!
# We keep the best from original tuning and explore NEW dimensions
PARAM_GRID = {
    # Keep best from original tuning (or near-best)
    'max_depth': [6, 8],  # Narrowed based on original results
    'learning_rate': [0.05, 0.1],  # Narrowed
    'min_child_weight': [3, 5],  # Narrowed

    # NEW PARAMETERS TO EXPLORE
    'reg_alpha': [0, 0.1, 0.5, 1.0],  # L1 regularization
    'reg_lambda': [1, 2, 5],  # L2 regularization
    'scale_pos_weight': [1.0, 1.2, 1.5],  # Class imbalance handling
}

# Fixed parameters
FIXED_PARAMS = {
    'n_estimators': 500,
    'subsample': 0.8,  # Fixed at best from original
    'colsample_bytree': 0.8,  # Fixed at best from original
    'gamma': 0.1,  # Fixed at best from original
    'random_state': 42,
    'eval_metric': 'auc',
...
log("=" * 70)
log("HYPERPARAMETER TUNING V2 - NEW PARAMETER SPACE")
log("=" * 70)
log(f"Started: {datetime.now()}")
log(f"\nExploring: reg_alpha, reg_lambda, scale_pos_weight")
...
for test_year in validation_years:
    ...
    log(f"Fold {test_year}: Train {len(folds[-1]['train_idx']):,}, Val {len(folds[-1]['val_idx']):,}, Test {len(folds[-1]['test_idx']):,}")
...
def evaluate_params(params, folds, df, feature_cols, thresholds=EVAL_THRESHOLDS, weights=THRESHOLD_WEIGHTS):
    ...
    for thresh in thresholds:
        ...
        for fp in fold_predictions:
            ...
            if len(filtered) == 0:
                fold_results.append({
                    'year': fp['year'], 'n_trades': 0, 'n_winning_trades': 0,
                    'win_rate': 0, 'pnl': 0, 'pf': 0, 'winners': 0, 'losers': 0,
                })
                continue
            ...
            fold_results.append({
                'year': fp['year'], 'n_trades': n_trades, 'n_winning_trades': n_winning_trades,
                'win_rate': win_rate, 'pnl': total_pnl, 'pf': pf,
        ...
        threshold_results[thresh] = {
            'pooled_pf': pooled_pf, 'weighted_wr': weighted_wr, 'total_pnl': total_pnl,
            'total_trades': total_trades_all, 'total_winning_trades': total_winning_trades,
            'total_winners_pips': total_winners_pips, 'total_losers_pips': total_losers_pips,
            'fold_results': fold_results,
        }

    weighted_pf = sum(threshold_results[t]['pooled_pf'] * w for t, w in zip(thresholds, weights))
    meets_min_trades = all(threshold_results[t]['total_trades'] >= MIN_TRADES_PER_THRESHOLD for t in thresholds)
    primary = threshold_results[0.75]

    return {
        'weighted_pf': weighted_pf,
        'pooled_pf': primary['pooled_pf'],
        'weighted_wr': primary['weighted_wr'],
...
log("\n" + "=" * 70)
log("GRID SEARCH V2 (exploring regularization & class weights)")
log("=" * 70)
...
for i, combo in enumerate(all_combinations):
    ...
    try:
        ...
        if result['weighted_pf'] > best_score and result['meets_min_trades']:
            ...
            log(f"  NEW BEST: Weighted PF={best_score:.2f}, WR={result['weighted_wr']:.1f}%, Trades@.70/.75/.80={trades_str}")
...
for rank, r in enumerate(results_sorted[:10], 1):
    ...
    log(f"{rank:<5} {r['weighted_pf']:>8.2f} {pf_75:>8.2f} {r['weighted_wr']:>6.1f}% {trades_75:>10,} | {params_str}")
...
log("\n" + "=" * 70)
log("BEST MODEL DETAILS (V2 Tuning)")
log("=" * 70)
...
log(f"{'Year':<6} {'Trades':>10} {'WR%':>8} {'PnL':>12} {'PF':>8}")
log("-" * 50)
for fr in best_result['fold_results']:
    log(f"{fr['year']:<6} {fr['n_trades']:>10,} {fr['win_rate']:>7.1f}% {fr['pnl']:>+12,.0f} {fr['pf']:>8.2f}")
...
# Original best params (from first tuning)
original_params = {
    'max_depth': 8,
    'learning_rate': 0.1,
    'min_child_weight': 5,
...
original_result = evaluate_params(original_params, folds, df, feature_cols, EVAL_THRESHOLDS, THRESHOLD_WEIGHTS)

log(f"\n{'Model':<20} {'Wtd PF':>8} {'PF@.75':>8} {'WR%':>7} {'Trades':>10} {'PnL':>12}")
log("-" * 75)
orig_75 = original_result['threshold_results'][0.75]['pooled_pf']
best_75 = best_result['threshold_results'][0.75]['pooled_pf']
log(f"{'Original Tuned':<20} {original_result['weighted_pf']:>8.2f} {orig_75:>8.2f} {original_result['weighted_wr']:>6.1f}% {original_result['total_trades']:>10,} {original_result['total_pnl']:>+12,.0f}")
log(f"{'V2 Tuned':<20} {best_result['weighted_pf']:>8.2f} {best_75:>8.2f} {best_result['weighted_wr']:>6.1f}% {best_result['total_trades']:>10,} {best_result['total_pnl']:>+12,.0f}")

improvement_pf = (best_result['weighted_pf'] - original_result['weighted_pf']) / original_result['weighted_pf'] * 100 if original_result['weighted_pf'] > 0 else 0
improvement_pnl = (best_result['total_pnl'] - original_result['total_pnl']) / abs(original_result['total_pnl']) * 100 if original_result['total_pnl'] != 0 else 0

log(f"\nImprovement over original: Weighted PF {improvement_pf:+.1f}%, PnL {improvement_pnl:+.1f}%")

# ============================================================================
# TRAIN FINAL MODEL IF BETTER
# ============================================================================

if best_result['weighted_pf'] > original_result['weighted_pf']:
    log("\n" + "=" * 70)
    log("TRAINING FINAL V2 TUNED MODEL")
    log("=" * 70)
    ...
    log(f"\nFinal model trained on {len(final_train_df):,} samples")

    # Save V2 tuned model (NEW FILE)
    model_path = MODEL_DIR / 'quality_xgb_m5_v2_tuned_v2.joblib'
    features_path = MODEL_DIR / 'quality_xgb_features_m5_v2_tuned_v2.pkl'
    ...
    results_path = MODEL_DIR / 'tuning_results_m5_v2.pkl'
    with open(results_path, 'wb') as f:
        pickle.dump(tuning_results, f)

    log(f"\nV2 Tuned model saved to: {model_path}")
    log(f"Features saved to: {features_path}")
    log(f"Tuning results saved to: {results_path}")
else:
    log("\nV2 Tuned model is NOT better than original. No new model saved.")
