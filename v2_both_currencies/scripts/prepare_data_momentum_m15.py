"""
Prepare Momentum Entry Dataset - M15
=====================================
...
log("=" * 70)
log("PREPARING MOMENTUM ENTRY DATASET (M15)")
log("=" * 70)
...
CROSS_LEVEL = 0.0
TARGET_LEVEL = 0.5
MAX_BARS_TO_TARGET = 100  # M15 bars
...
def create_dataset_for_pair(base_ccy, quote_ccy):
    ...
    price_m15 = price_df['Close'].resample('15min').last().dropna()
    ...
    # - Lower TF (M5): shift by 1 bar of M5, then ffill to M15
    # - Higher TF (M30, H1, H4): shift by 1 bar of THAT TF, then ffill to M15

    # M5 (lower TF) - shift 1 M5 bar before ffill
    if base_mfc['M5'] is not None:
        df['base_m5'] = base_mfc['M5'].shift(1).reindex(df.index, method='ffill')
    if quote_mfc['M5'] is not None:
        df['quote_m5'] = quote_mfc['M5'].shift(1).reindex(df.index, method='ffill')

    # M30, H1, H4 (higher TFs) - shift 1 bar of THAT TF before ffill
    for tf in ['M30', 'H1', 'H4']:
    ...
    # BASE TF (M15): shift by 1 M15 bar for trigger detection
    df['base_m15_shifted'] = df['base_m15'].shift(1)
    df['quote_m15_shifted'] = df['quote_m15'].shift(1)
    ...
    # M15 velocity: diff of shifted M15 (1 M15 bar change)
    df['base_vel_m15'] = df['base_m15_shifted'].diff()
    df['quote_vel_m15'] = df['quote_m15_shifted'].diff()
    ...
    # Additional momentum features on M15
    df['base_vel3_m15'] = df['base_m15_shifted'].diff(3) / 3
    df['base_vel5_m15'] = df['base_m15_shifted'].diff(5) / 5
    df['quote_vel3_m15'] = df['quote_m15_shifted'].diff(3) / 3
    df['quote_vel5_m15'] = df['quote_m15_shifted'].diff(5) / 5
    ...
    # Convert to numpy
    base_m15_arr = df['base_m15'].values
    quote_m15_arr = df['quote_m15'].values
    base_shifted = df['base_m15_shifted'].values
    quote_shifted = df['quote_m15_shifted'].values
    ...
    cross_up_base = (base_shifted <= CROSS_LEVEL) & (df['base_m15_shifted'].shift(-1) > CROSS_LEVEL)
    cross_up_idx = np.where(cross_up_base.values[:-1])[0]

    entries = process_crossings(
        df, cross_up_idx, base_m15_arr, price_arr, base_vel,
    ...
    cross_down_base = (base_shifted >= CROSS_LEVEL) & (df['base_m15_shifted'].shift(-1) < CROSS_LEVEL)
    cross_down_idx = np.where(cross_down_base.values[:-1])[0]

    entries = process_crossings(
        df, cross_down_idx, base_m15_arr, price_arr, base_vel,
        direction=-1, trigger='base', trigger_ccy=base_ccy,
        base_ccy=base_ccy, quote_ccy=quote_ccy, pair=pair,
        target=-TARGET_LEVEL
    )
    ...
    # Quote cross UP -> SELL pair
    cross_up_quote = (quote_shifted <= CROSS_LEVEL) & (df['quote_m15_shifted'].shift(-1) > CROSS_LEVEL)
    cross_up_idx = np.where(cross_up_quote.values[:-1])[0]

    entries = process_crossings(
        df, cross_up_idx, quote_m15_arr, price_arr, quote_vel,
    ...
    # Quote cross DOWN -> BUY pair
    cross_down_quote = (quote_shifted >= CROSS_LEVEL) & (df['quote_m15_shifted'].shift(-1) < CROSS_LEVEL)
    cross_down_idx = np.where(cross_down_quote.values[:-1])[0]

    entries = process_crossings(
        df, cross_down_idx, quote_m15_arr, price_arr, quote_vel,
        direction=1, trigger='quote', trigger_ccy=quote_ccy,
        base_ccy=base_ccy, quote_ccy=quote_ccy, pair=pair,
        target=-TARGET_LEVEL
    )
...
def process_crossings(df, indices, mfc_arr, price_arr, vel_arr,
    ...
    results['base_vel3_m15'] = df['base_vel3_m15'].values[indices]
    results['base_vel5_m15'] = df['base_vel5_m15'].values[indices]
    results['quote_vel3_m15'] = df['quote_vel3_m15'].values[indices]
    results['quote_vel5_m15'] = df['quote_vel5_m15'].values[indices]
...
def main():
    ...
    # Save
    output_path = OUTPUT_DIR / 'momentum_entry_data_m15.pkl'
    output = {
        'data': final_df,
        'config': {
