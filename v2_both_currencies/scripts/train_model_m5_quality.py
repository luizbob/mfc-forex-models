"""
Train Quality Model - V2 Both Currencies (M5)
==============================================
Target: is_quality_trade = profitable AND max_dd <= 50 pips
Risk-adjusted target that filters for low-drawdown winners.
"""
import warnings
...
MAX_DD_PIPS = 50  # 500 points

def log(msg=""):
    print(msg, flush=True)

log("=" * 70)
log("TRAINING QUALITY MODEL (LOW DD + PROFITABLE) - V2 M5")
log("=" * 70)
log(f"Started: {datetime.now()}")
log(f"Max drawdown threshold: {MAX_DD_PIPS} pips ({MAX_DD_PIPS * 10} points)")
...
df.loc[jpy_mask, 'max_profit_pips'] = df.loc[jpy_mask, 'max_profit_pips'] / 100

# Create quality target: profitable AND low drawdown
df['is_quality_trade'] = ((df['exit_pnl_pips'] > 0) & (df['max_dd_pips'] <= MAX_DD_PIPS)).astype(int)
...
# Target rates
log(f"\nTarget rates:")
log(f"  Quality trade (profitable + DD<={MAX_DD_PIPS}p):")
log(f"    Train: {train_df['is_quality_trade'].mean()*100:.1f}%")
log(f"    Val:   {val_df['is_quality_trade'].mean()*100:.1f}%")
log(f"    Test:  {test_df['is_quality_trade'].mean()*100:.1f}%")
log(f"  For comparison - Profitable only:")
...
y_train = train_df['is_quality_trade'].values
y_val = val_df['is_quality_trade'].values
y_test = test_df['is_quality_trade'].values
...
# Save model
model_path = MODEL_DIR / 'quality_xgb_m5_v2_quality.joblib'
features_path = MODEL_DIR / 'quality_xgb_features_m5_v2_quality.pkl'
...
log("\n" + "=" * 70)
log("BACKTEST ON TEST SET (Real PnL)")
log("=" * 70)

log(f"\n| Threshold | Trades | Win% | Avg PnL | Total PnL | PF   | Med DD | Trades/Day |")
log(f"|-----------|--------|------|---------|-----------|------|--------|------------|")
...
for thresh in [0.5, 0.6, 0.7, 0.8, 0.9]:
    ...
    med_dd = f['max_dd_pips'].median()
    tpd = len(f) / days if days > 0 else 0
    log(f"| {thresh:^9} | {len(f):>6,} | {wr:>4.1f}% | {avg:>7.1f} | {total:>9.0f} | {pf:>4.2f} | {med_dd:>6.1f} | {tpd:>10.1f} |")

# Compare with PnL model
log("\n" + "=" * 70)
log("COMPARISON: QUALITY MODEL vs PNL MODEL")
log("=" * 70)

pnl_model_path = MODEL_DIR / 'quality_xgb_m5_v2_pnl.joblib'
if pnl_model_path.exists():
    pnl_model = joblib.load(pnl_model_path)
    test_df['pnl_pred'] = pnl_model.predict_proba(X_test)[:, 1]

    log(f"\n| Model   | Thresh | Trades | Win% | Avg PnL | Total PnL | PF   | Med DD |")
    log(f"|---------|--------|--------|------|---------|-----------|------|--------|")

    for thresh in [0.7, 0.8, 0.9]:
        # Quality model
        fq = test_df[test_df['pred_prob'] >= thresh]
        if len(fq) > 0:
            pnl = fq['exit_pnl_pips']
            wr = (pnl > 0).mean() * 100
            w = pnl[pnl > 0].sum()
            l = abs(pnl[pnl <= 0].sum())
            pf = w / l if l > 0 else float('inf')
            log(f"| Quality | {thresh:>5} | {len(fq):>6,} | {wr:>4.1f}% | {pnl.mean():>7.1f} | {pnl.sum():>9.0f} | {pf:>4.2f} | {fq['max_dd_pips'].median():>6.1f} |")

        # PnL model
        fp = test_df[test_df['pnl_pred'] >= thresh]
        if len(fp) > 0:
            pnl = fp['exit_pnl_pips']
            wr = (pnl > 0).mean() * 100
            w = pnl[pnl > 0].sum()
            l = abs(pnl[pnl <= 0].sum())
            pf = w / l if l > 0 else float('inf')
            log(f"| PnL     | {thresh:>5} | {len(fp):>6,} | {wr:>4.1f}% | {pnl.mean():>7.1f} | {pnl.sum():>9.0f} | {pf:>4.2f} | {fp['max_dd_pips'].median():>6.1f} |")

        log(f"|---------|--------|--------|------|---------|-----------|------|--------|")

# By trigger type
log("\n" + "=" * 70)
log("BY TRIGGER TYPE (Threshold 0.8)")
log("=" * 70)

filtered = test_df[test_df['pred_prob'] >= 0.8].copy()

log(f"\n| Trigger | Trades | Win% | Avg PnL | Total PnL | PF   | Med DD |")
log(f"|---------|--------|------|---------|-----------|------|--------|")
for trigger in ['base', 'quote']:
    ...
    if len(s) > 0:
        ...
        l = abs(p[p <= 0].sum())
        pf = w / l if l > 0 else float('inf')
        log(f"| {trigger:^7} | {len(s):>6,} | {wr:>4.1f}% | {p.mean():>7.1f} | {p.sum():>9.0f} | {pf:>4.2f} | {s['max_dd_pips'].median():>6.1f} |")

# By trigger currency
log("\n" + "=" * 70)
log("BY TRIGGER CURRENCY (Threshold 0.8)")
log("=" * 70)

log(f"\n| Currency | Trades | Win% | Avg PnL | Total PnL | PF   | Med DD |")
log(f"|----------|--------|------|---------|-----------|------|--------|")
for ccy in ['EUR', 'USD', 'GBP', 'JPY', 'CHF', 'CAD', 'AUD', 'NZD']:
    ...
    if len(s) > 0:
        ...
        l = abs(p[p <= 0].sum())
        pf = w / l if l > 0 else float('inf')
        log(f"| {ccy:^8} | {len(s):>6,} | {wr:>4.1f}% | {p.mean():>7.1f} | {p.sum():>9.0f} | {pf:>4.2f} | {s['max_dd_pips'].median():>6.1f} |")
