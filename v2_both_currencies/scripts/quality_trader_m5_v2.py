"""
Quality Entry Trader V2 - Both Currency Triggers (M5)
=====================================================
Triggers on BOTH base AND quote currency extremes.
Uses M5 timeframe for entries/exits.
...
# Trading Parameters
LOT_SIZE = 0.01
MAGIC_NUMBER = 150050  # V2 M5 magic number
MAX_SPREAD_PIPS = 5
MAX_POSITIONS = 30
...
class QualityModelManager:
    ...
    def load(self) -> bool:
        model_path = self.model_dir / 'quality_xgb_m5_v2_pnl.joblib'
        features_path = self.model_dir / 'quality_xgb_features_m5_v2_pnl.pkl'

        if not model_path.exists() or not features_path.exists():
            log.error(f"V2 M5 model not found")
            return False
        ...
        try:
            ...
            log.info(f"Loaded V2 M5 quality model: {len(self.feature_cols)} features")
            return True
...
class MFCDataReader:
    ...
    def get_mfc_features(self, base: str, quote: str, direction: str, trigger: str) -> dict:
        """Get features for V2 M5 model (momentum on M5)."""
        data = self.read()
        ...
        for p in self.get_positions():
            if p.symbol == symbol:
                try:
                    ...
                    if len(parts) >= 3 and parts[0] == 'V2M5':
...
class QualityEntryStrategyV2M5:
    def __init__(self):
    ...
    def initialize(self) -> bool:
        log.info("=" * 60)
        log.info("QUALITY ENTRY TRADER V2 - BOTH CURRENCIES (M5)")
        log.info("=" * 60)
    ...
    def process_bar(self):
        ...
        for pair, base, quote in ALL_PAIRS:
            ...
            if self.trader.has_position(symbol):
                ...
                if direction:
                    signals += 1
                    comment = f"V2M5_{trigger.upper()}_{trigger_ccy}_{direction.upper()}"
...
def main():
    strategy = QualityEntryStrategyV2M5()
    try:
