"""
Prepare Quality Entry Dataset - V2 Both Currencies (H1)
=======================================================
...
log("=" * 70)
log("PREPARING QUALITY ENTRY DATASET - V2 BOTH CURRENCIES (H1)")
log("=" * 70)
...
RETURN_TARGET = 0.0
MAX_BARS_TO_RETURN = 200  # H1 bars
...
def create_dataset_for_pair(base_ccy, quote_ccy):
    ...
    price_h1 = price.resample('1h').last().dropna()

    # Create base dataframe on H1
    ...
    # This prevents data leakage for both higher TFs (H4) and lower TFs (M5, M15, M30)
    ...
    # Apply shift(1) on H1 grid - use previous H1 bar's data for trigger/features
    for tf in ['m5', 'm15', 'm30', 'h1', 'h4']:
    ...
    # Additional H1 features (momentum for trading timeframe)
    df['base_vel2_h1'] = df['base_h1_shifted'].diff(2)
    df['base_acc_h1'] = df['base_vel_h1'].diff()
    df['quote_vel2_h1'] = df['quote_h1_shifted'].diff(2)
    df['quote_acc_h1'] = df['quote_vel_h1'].diff()
    ...
    buy_mask_base = df['base_h1_shifted'] <= -MFC_EXTREME_THRESHOLD
    ...
    sell_mask_base = df['base_h1_shifted'] >= MFC_EXTREME_THRESHOLD
    ...
    sell_mask_quote = df['quote_h1_shifted'] <= -MFC_EXTREME_THRESHOLD
    ...
    buy_mask_quote = df['quote_h1_shifted'] >= MFC_EXTREME_THRESHOLD
...
def label_entry(df, idx, direction, trigger, trigger_ccy):
    ...
    try:
        ...
        # Entry at NEXT bar's open price (realistic execution)
        entry_price = df.iloc[pos+1]['price']
        ...
        if trigger == 'base':
            ...
            entry_mfc = df.iloc[pos]['quote_h1_shifted']
            future_mfc = df.iloc[pos+1:pos+MAX_BARS_TO_RETURN+1]['quote_h1']
        ...
        features['quote_vel2_h1'] = row['quote_vel2_h1']
        features['quote_acc_h1'] = row['quote_acc_h1']
...
def main():
    ...
    # Save
    output_path = OUTPUT_DIR / 'quality_entry_data_h1_v2.pkl'
    output = {
        'data': final_df,
        'config': {
