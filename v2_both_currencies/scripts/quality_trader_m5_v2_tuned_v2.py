"""
Quality Entry Trader V2 - Tuned V2 Model (M5)
==============================================
Uses the regularization-tuned model (V2 tuning).
Parameters: max_depth=6, learning_rate=0.1, with regularization.

Best balance of quality and volume:
  @ 0.75: 80.3% WR Net, PF Net=7.16, +32,356 PnL Net (2x trades vs Original)
...
# Trading Parameters
LOT_SIZE = 0.01
MAGIC_NUMBER = 150055  # V2 Tuned V2 model magic number
MAX_SPREAD_PIPS = 5
MAX_POSITIONS = 100
...
class TradeLogger:
    ...
    def __init__(self, log_dir: Path):
        ...
        self.trades_file = self.log_dir / "trades_v2tuned_v2.csv"
        self._init_file()
...
# ============================================================================
# QUALITY MODEL MANAGER (V2 Tuned Model)
# ============================================================================
...
class QualityModelManager:
    ...
    def load(self) -> bool:
        model_path = self.model_dir / 'quality_xgb_m5_v2_tuned_v2.joblib'
        features_path = self.model_dir / 'quality_xgb_features_m5_v2_tuned_v2.pkl'

        if not model_path.exists() or not features_path.exists():
            log.error(f"V2 Tuned model not found at {model_path}")
            return False
        ...
        try:
            ...
            log.info(f"Loaded V2 Tuned model: {len(self.feature_cols)} features")
            return True
        ...
        for p in self.get_positions():
            if p.symbol == symbol:
                try:
                    ...
                    if len(parts) >= 3 and parts[0] == 'V2TV2':
...
class QualityEntryStrategyV2TunedV2:
    def __init__(self):
    ...
    def initialize(self) -> bool:
        log.info("=" * 60)
        log.info("QUALITY ENTRY TRADER V2 - TUNED V2 MODEL (M5)")
        log.info("=" * 60)
        ...
        log.info(f"Model: V2 Tuned (max_depth=6, lr=0.1, regularization)")
    ...
    def process_bar(self):
        ...
        for pair, base, quote in ALL_PAIRS:
            ...
            if self.trader.has_position(symbol):
                ...
                if direction:
                    signals += 1
                    comment = f"V2TV2_{trigger.upper()}_{trigger_ccy}_{direction.upper()}"
...
def main():
    strategy = QualityEntryStrategyV2TunedV2()
    try:
