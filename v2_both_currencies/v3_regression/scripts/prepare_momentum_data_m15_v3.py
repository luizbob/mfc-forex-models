"""
Prepare Momentum Dataset - M15 Signal - V2
===========================================
Exit when velocity drops below threshold (not fixed time).
"""
import warnings
...
log("=" * 70)
log("PREPARING MOMENTUM DATASET - M15 V2 (velocity exit)")
log("=" * 70)
...
EXIT_THRESHOLD = 0.02  # Even less sensitive exit (quarter of entry threshold)
MAX_HOLD_BARS = 24  # Maximum 6 hours, but exit earlier if velocity drops
...
def create_dataset_for_pair(base_ccy, quote_ccy):
    ...
    # Find signals with velocity-based exit
    all_entries = []
    ...
    base_vel_shifted = df['base_vel_m15'].values  # For entry detection (shifted)
    quote_vel_shifted = df['quote_vel_m15'].values
    base_vel_raw = df['base_vel_raw'].values  # For exit tracking (raw)
    ...
    while i < n - MAX_HOLD_BARS - 1:
        ...
        if base_vel > VELOCITY_THRESHOLD:
            ...
            trigger_type = 'base'
            expected_sign = -1
        # Quote currency trigger (only if base didn't trigger)
        ...
        for j in range(entry_idx, min(entry_idx + MAX_HOLD_BARS, n)):
            ...
            # Exit conditions:
            # 1. Velocity dropped below threshold
            # 2. Velocity reversed direction
            if trigger_type == 'base':
                if signal == 'buy':
                    # Was going up, exit if velocity drops or reverses
                    if current_vel < EXIT_THRESHOLD:
                        exit_idx = j
                        break
                else:
                    # Was going down, exit if velocity drops or reverses
                    if current_vel > -EXIT_THRESHOLD:
                        exit_idx = j
                        break
            else:  # quote trigger
                if signal == 'sell':
                    # Quote was going up, exit if drops
                    if current_vel < EXIT_THRESHOLD:
                        exit_idx = j
                        break
                else:
                    # Quote was going down, exit if drops
                    if current_vel > -EXIT_THRESHOLD:
                        exit_idx = j
                        break
...
def main():
    ...
    # Save
    output_path = OUTPUT_DIR / 'momentum_data_m15_v3.pkl'
    output = {
        'data': final_df,
        'config': {
