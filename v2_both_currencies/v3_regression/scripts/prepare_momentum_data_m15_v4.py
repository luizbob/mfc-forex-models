"""
Prepare Momentum Dataset - M15 Signal - V4
===========================================
V4: Only trigger when MFC crosses FROM extreme INTO box (not just high velocity)
"""
import warnings
...
log("=" * 70)
log("PREPARING MOMENTUM DATASET - M15 V4 (box crossing only)")
log("=" * 70)
...
def create_dataset_for_pair(base_ccy, quote_ccy):
    ...
    # Store RAW MFC values for box crossing detection
    df['base_mfc_raw'] = base_mfc['M15']
    df['quote_mfc_raw'] = quote_mfc['M15']
    df['base_mfc_prev'] = base_mfc['M15'].shift(1)  # Previous bar MFC
    df['quote_mfc_prev'] = quote_mfc['M15'].shift(1)

    # Store RAW (unshifted) velocity for exit tracking
    df['base_vel_raw'] = base_mfc['M15'].diff()
    df['quote_vel_raw'] = quote_mfc['M15'].diff()

    # Shift all timeframes for features (no look-ahead)
    for tf in ['M15', 'M30', 'H1', 'H4']:
        tf_lower = tf.lower()
    ...
    df['base_in_box'] = ((df['base_m15'] >= BOX_LOWER) & (df['base_m15'] <= BOX_UPPER)).astype(int)
    df['quote_in_box'] = ((df['quote_m15'] >= BOX_LOWER) & (df['quote_m15'] <= BOX_UPPER)).astype(int)
    df['base_dist_box'] = df['base_m15'].apply(lambda x: max(0, abs(x) - BOX_UPPER))
    df['quote_dist_box'] = df['quote_m15'].apply(lambda x: max(0, abs(x) - BOX_UPPER))
    df['h4_base_confirms_up'] = ((df['base_h4'] < BOX_UPPER) & (df['base_vel_h4'] > 0)).astype(int)
    df['h4_base_confirms_down'] = ((df['base_h4'] > BOX_LOWER) & (df['base_vel_h4'] < 0)).astype(int)
    df['h4_quote_confirms_up'] = ((df['quote_h4'] < BOX_UPPER) & (df['quote_vel_h4'] > 0)).astype(int)
    df['h4_quote_confirms_down'] = ((df['quote_h4'] > BOX_LOWER) & (df['quote_vel_h4'] < 0)).astype(int)

    df = df.dropna()
    log(f"  Data: {len(df)} M15 bars")

    # Find signals with BOX CROSSING condition
    all_entries = []
    ...
    # For box crossing detection (using shifted values - what we knew at decision time)
    base_mfc_prev = df['base_m15'].values  # This is already shifted by 1
    quote_mfc_prev = df['quote_m15'].values
    # Current MFC (after the move) - need to look at raw values shifted by appropriate amount
    base_mfc_curr = df['base_mfc_raw'].values
    quote_mfc_curr = df['quote_mfc_raw'].values
    ...
    while i < n - MAX_HOLD_BARS - 1:
        ...
        # Previous bar MFC (what we knew before the move)
        base_prev = base_mfc_prev[i]
        quote_prev = quote_mfc_prev[i]

        # Current bar MFC (after the move happened)
        base_curr = base_mfc_curr[i]
        quote_curr = quote_mfc_curr[i]

        signal = None
        trigger_type = None
        expected_sign = 0

        # BASE CURRENCY: Check for box crossing with high velocity
        if abs(base_vel) > VELOCITY_THRESHOLD:
            vel_trigger_count += 1

            # Coming DOWN from above box INTO box
            if base_prev > BOX_UPPER and base_curr <= BOX_UPPER and base_curr >= BOX_LOWER:
                signal = 'sell'
                trigger_type = 'base'
                expected_sign = -1
                box_cross_count += 1
            # Coming UP from below box INTO box
            elif base_prev < BOX_LOWER and base_curr >= BOX_LOWER and base_curr <= BOX_UPPER:
                signal = 'buy'
                trigger_type = 'base'
                expected_sign = 1
                box_cross_count += 1

        # QUOTE CURRENCY: Check for box crossing with high velocity (only if base didn't trigger)
        if signal is None and abs(quote_vel) > VELOCITY_THRESHOLD:
            vel_trigger_count += 1

            # Quote coming DOWN from above → pair goes UP (buy)
            if quote_prev > BOX_UPPER and quote_curr <= BOX_UPPER and quote_curr >= BOX_LOWER:
                signal = 'buy'
                trigger_type = 'quote'
                expected_sign = 1
                box_cross_count += 1
            # Quote coming UP from below → pair goes DOWN (sell)
            elif quote_prev < BOX_LOWER and quote_curr >= BOX_LOWER and quote_curr <= BOX_UPPER:
                signal = 'sell'
                trigger_type = 'quote'
                expected_sign = -1
                box_cross_count += 1
        ...
        # Find exit: when velocity drops below threshold OR max bars
        exit_idx = entry_idx
    ...
    if not all_entries:
        log(f"  No entries found (vel triggers: {vel_trigger_count}, box crosses: {box_cross_count})")
        return None
...
def main():
    ...
    for low, high, label in [(0.08, 0.12, '0.08-0.12'), (0.12, 0.18, '0.12-0.18'),
        ...
        if len(subset) > 0:
            log(f"  {label}: {len(subset):,}, {subset['is_profitable'].mean()*100:.1f}%, {subset['net_pips'].mean():+.2f}, hold {subset['bars_held'].mean():.1f}")
    ...
    # Save
    output_path = OUTPUT_DIR / 'momentum_data_m15_v4.pkl'
    output = {
        'data': final_df,
        'config': {
            ...
            'entry_type': 'box_crossing',
            'base_timeframe': 'M15',
            'version': 'v4_momentum_m15_box_cross',
        }
    }
