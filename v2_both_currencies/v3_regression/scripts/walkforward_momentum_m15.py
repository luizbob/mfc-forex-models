"""
Walk-Forward Validation - Momentum Model M15
=============================================
Tests model performance across multiple time periods to ensure robustness.
"""
import warnings
...
log("=" * 70)
log("WALK-FORWARD VALIDATION - MOMENTUM MODEL M15")
log("=" * 70)
...
# Train on all data up to test_year-1, test on test_year
...
for test_year in test_years:
    ...
    log(f"Train: {len(X_train):,} samples ({df[train_mask]['year'].min()}-{test_year-1})")
    ...
    # By bars held (at 0.45 threshold)
...
if results:
    ...
    log(f"  Std dev of avg pips: {np.std([r['avg'] for r in results]):.2f}")
